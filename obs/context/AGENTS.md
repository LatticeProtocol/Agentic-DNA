...
Curated, validated context files that agents load when they need domain expertise. The context library lives in `obs/context/` as part of the knowledge layer.
...
1. **Check** `obs/context/` for relevant topics before starting domain-specific tasks
...
All subtypes coexist in `obs/context/` organized by topic. The subtype informs how agents use the content, not where it lives.

Template: `ops/templates/template_context.md`
